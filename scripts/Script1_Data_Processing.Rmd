...
```{r read.dat}
# Test
# test read_csv("/Users/Dell/Documents/GitHub/Pathogen-Data-Analysis-Training/survey_data_simulated.csv")
...
table (microbio_clean$sample_type) # frequency table for categories
summary(microbio_clean$soil_moisture) # summary for continuous variables
...
microbio_clean_v2 <- microbio_clean %>%  # add version 2
  mutate(
    # Add total coliform and E. coli MPN estimation (with 95% confidence limits) code here
    tc_mpn = quantify_mpn(total_coliform_large_cell_count, 
                          total_coliform_small_cell_count,
                          method = "qt-2000"),
    tc_mpn_lo = quantify_95lo(total_coliform_large_cell_count, 
                          total_coliform_small_cell_count,
                          method = "qt-2000"),
    tc_mpn_hi = quantify_95hi(total_coliform_large_cell_count, 
                          total_coliform_small_cell_count,
                          method = "qt-2000"),
...
```{r mpn.check}
# view the changes or new variables and incorporate it to original dataset
...
#check frequency of tc detect through sample type
...
ggplot(microbio_clean2, aes(x = log_adjusted_esbl_cfu, fill = sample_type)) +
  ...
  theme_minimal()
  ggplot(microbio_clean2, aes(x = esbl_e_coli_cfu, fill = sample_type)) +
  geom_histogram(bins = 30, alpha = 0.6, position = "identity") +
  theme_minimal()
```
...
microbio_clean2 <- microbio_clean2 %>%
  mutate(
    ...
    across(c(tc_mpn, ar_tc_mpn, ecoli_mpn, ar_ecoli_mpn),
                ...
                .names = "{.col}_adj")) %>% #looping through and adding _adj to all variable names
  mutate(
...
#add code to add 1 to adjusted esbl cfu to shift values from neg to positive
#for mpn we already did log10() +1, but for esbl we only did log10() without adding 1, so we had neg values for it, by adding 1 we shift it up by 1 here
microbio_clean2$log_adjusted_esbl_e_coli_cfu_adj <-
  microbio_clean2$log_adjusted_esbl_e_coli_cfu_adj +1
hist(microbio_clean2$log_adjusted_adjusted_esbl_e_coli_cfu_adj)
```
